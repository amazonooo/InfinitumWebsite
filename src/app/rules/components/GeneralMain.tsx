...
const GeneralMain: FC = () => {
	...
	// useEffect(() => {
	// 	const hasVisited = localStorage.getItem('GeneralMainVisited')
	// 	if (!hasVisited) {
	// 		setIsFirstRender(true)
	// 		localStorage.setItem('GeneralMainVisited', 'true')
	// 	}
	// }, [])
  ...
  useEffect(() => {
		if (isFirstRender) {
			const timeout = setTimeout(() => {
				const element = document.querySelector('.rules-main')
				element?.classList.add('rules-main-animate')
			}, 300)
	...
	return (
		<div
			className={`relative rounded-lg border border-white/[0.2] shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] bg-[#09090B] ${
				isFirstRender ? 'rules-main' : 'rules-main-animate'
			}`}
		>
			<div className='p-8'>

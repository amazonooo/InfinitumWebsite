...
const FloatingDockDesktop = ({
	...
	return (
		<motion.div
			...
			className={cn(
				'mx-auto hidden md:flex h-16 gap-4 items-end  rounded-2xl bg-main-black border border-white/[0.2] px-4 pb-3',
				className
			)}
		>
			{items.map(item => (
				...
				<AnimatePresence>
					{hovered && (
						<motion.div
							...
							className='px-2.5 py-1 whitespace-pre rounded-md border bg-neutral-800 border-neutral-900 text-white absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs'
						>
							{title}
						</motion.div>
					)}

...
import { useInitialScrollNews } from '@/hooks/useInitialScrollNews'
...
export default function News({ initialPosts = [], limit }: IPostsItem) {
	const { posts, hasMore, loading, ref, page, setPage } = useInitialScrollNews({ initialPosts, limit })
	// const [isHovered, setIsHovered] = useState(false)

	const router = useRouter()

	const handlePageChange = (newPage: number) => {
		if (newPage < 1 || !hasMore || loading) return
		setPage(newPage)
		router.push(`/news?page=${newPage}&limit=${limit}`, undefined) // Убираем второй аргумент с shallow
	}
			...
			<m.div initial='hidden' animate='visible'>
				...
				>
					...
					{/* <div className='flex items-center justify-center flex-col w-full'>
						...
						{!hasMore && <p className='text-center'>Больше новостей нет</p>}
					</div> */}
